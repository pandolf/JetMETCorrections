...
int main(int argc, char* argv[]) { 
  ...
  if( argc != 6 ) {
    std::cout << "USAGE: ./do2ndLevel_PhotonJet_batch [dataset] [recoType] [jetAlgo] [inputFileList] [flags]" << std::endl;
    exit(23);
  }
...
void doSingleLoop(std::string inputFileList, std::string dataset, std::string recoType, std::string jetAlgo, std::string flags) {
  ...
  TreeAnalyzer_DiJet* t = new TreeAnalyzer_DiJet(dataset, recoType, jetAlgo, flags);
