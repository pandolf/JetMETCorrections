...
if PDname=="EG" or PDname=="Photon":
  diskoutputdir = "/shome/pandolf/DATA/" + dataset_path
else:
  diskoutputdir = "/shome/pandolf/MC/Summer11/" + dataset_path
...
#    inputfile.close()
    ...
    #os.system("echo qsub -q "+queue+" -o "+dir+"/log/"+dataset_name+"_"+str(ijob)+".log "+pwd+"/"+outputname)
    #out = os.popen("echo qsub -q "+queue+" -o "+dir+"/log/"+dataset_name+"_"+str(ijob)+".log "+pwd+"/"+outputname+" -copyInput="+dataset_name+"_"+str(ijob))
    os.system("ls -lah "+pwd+"/"+outputname)
    command = "qsub -q "+queue+" -o "+pwd+"/"+dir+"/log/"+dataset_name+"_"+str(ijob)+".log  -e "+pwd+"/"+dir+"/log/"+dataset_name+"_"+str(ijob)+".err "+pwd+"/"+outputname+" -copyInput="+dataset_name+"_"+str(ijob)
    print command
    os.system(command)
    ijob = ijob+1
    continue
