...
void TreeFinalizerC_QGStudies::finalize() {
  ...
  std::string algoType = (recoType_=="calo") ? jetAlgo_ : recoType_+jetAlgo_;
  if( recoType_=="jpt"&&jetAlgo_=="akt5" ) algoType="jptak5"; 
  ...
  outfileName = outfileName + "_"+algoType;;
  ...
  if( recoType_ == "pf" ) {
    ...
    tree_->SetBranchAddress("phiMet", &phiMet);
  } else if( recoType_ == "jpt" ) {
    tree_->SetBranchAddress("etcMet", &eMet);
    tree_->SetBranchAddress("phitcMet", &phiMet);
  }
