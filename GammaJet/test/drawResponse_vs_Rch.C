...
void drawResponse_vs_Rch(int iBin) {
  ...
  TFile* file = TFile::Open("Projections_RchCalibration/endcap.root");

  char name[50];
  sprintf(name, "ptbin%d/fit_%d", iBin, iBin);

  TProfile* gr_response_GEN = (TGraphErrors*)file->Get(name);
